    ...
    let account_ids = match provider {
        ...
        "xai" => state.xai_accounts.list_accounts().await.map(|items| {
            items
                .into_iter()
                .filter(|item| {
                    matches!(item.status, crate::xai::XaiAccountStatus::Active)
                        || (matches!(item.status, crate::xai::XaiAccountStatus::Expired)
...
const XAI_TOKEN_AUTH_HEADER: HeaderName =
    HeaderName::from_static(crate::xai::CLI_TOKEN_AUTH_HEADER);
const XAI_CLIENT_VERSION_HEADER: HeaderName =
    HeaderName::from_static(crate::xai::CLI_CLIENT_VERSION_HEADER);
...
pub(super) fn xai_request_url(upstream_path_with_query: &str) -> String {
    let base_url = if is_xai_official_api_path(upstream_path_with_query) {
        crate::xai::OFFICIAL_API_BASE_URL
    } else {
...
pub(super) fn enforce_xai_request_headers(
    ...
    request_headers.insert(
        ...
        HeaderValue::from_static(crate::xai::CLI_TOKEN_AUTH_VALUE),
    );
    request_headers.insert(
        XAI_CLIENT_VERSION_HEADER,
        HeaderValue::from_static(crate::xai::CLI_CLIENT_VERSION),
    );
    request_headers.insert(
        USER_AGENT,
        HeaderValue::from_static(crate::xai::CLI_USER_AGENT),
    );
    request_headers.insert(CONTENT_TYPE, HeaderValue::from_static("application/json"));
    request_headers.insert(
        ACCEPT,
