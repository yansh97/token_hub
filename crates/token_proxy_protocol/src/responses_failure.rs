...
use crate::responses_sequence::ResponsesEventSequence;
...
pub struct StreamNormalization {
    ...
    pub added_created_at: bool,
    pub added_model: bool,
    pub repaired_response_shape: bool,
}
...
pub struct HttpErrorNormalization {
    pub body: Bytes,
    pub changed: bool,
}

/// Repairs only the required OpenAI error envelope fields and preserves upstream details.
pub fn normalize_http_error(status: StatusCode, body: &Bytes) -> HttpErrorNormalization {
...
pub fn failed_event(message: &str, model: Option<&str>, sequence_number: u64) -> Value {
    let mut value = json!({
