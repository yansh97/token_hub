...
pub(super) async fn build_buffered_response(
    ...
    .await;
    let xai_forbidden_scope = (context.provider == "xai")
        .then(|| classify_http_forbidden(status.as_u16(), &bytes))
        .flatten();
    let xai_forbidden_code = xai_forbidden_scope.and_then(|_| payload_error_code(&bytes));
    if let Some(scope) = xai_forbidden_scope {
        tracing::warn!(
            ...
            forbidden_code = ?xai_forbidden_code,
            "classified xAI forbidden response"
        );
    }
    ...
    let is_request_policy_rejection =
        xai_forbidden_scope == Some(XaiForbiddenScope::RequestContentPolicy);
    let response_error = if is_request_policy_rejection {
        // 策略错误可能回显 prompt/media；日志只保留固定摘要和安全 code。
        context.request_body = None;
        Some(match xai_forbidden_code.as_deref() {
            Some(code) => format!("xAI request rejected by content policy ({code})."),
            None => "xAI request rejected by content policy.".to_string(),
        })
    } else {
        response_error_for_status(status, &bytes)
    };
