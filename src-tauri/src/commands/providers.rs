...
use token_proxy_core::provider_accounts::{
...
async fn apply_runtime_account_cooldowns(
    ...
    let xai_account_ids = items
        .iter()
        .filter(|item| {
            item.provider_kind == token_proxy_core::provider_accounts::ProviderAccountKind::Xai
                && item.status == token_proxy_core::provider_accounts::ProviderAccountStatus::Active
        })
        .map(|item| item.account_id.clone())
        .collect::<Vec<_>>();
    ...
    for item in items.iter_mut() {
        ...
        let is_cooling = match item.provider_kind {
            ...
            token_proxy_core::provider_accounts::ProviderAccountKind::Xai => {
...
pub async fn providers_delete_accounts(
    ...
    let items = token_proxy_core::provider_accounts::list_accounts_snapshot(
        paths.inner().as_ref(),
    ...
    token_proxy_core::provider_accounts::delete_accounts(
        paths.inner().as_ref(),
